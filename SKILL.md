...
| 分类 | 判定标准 |
|------|---------|
| Feature / 优化 | 新接口、新机制、新算法/策略、性能优化、架构重构、LSF/MM/BPF |
| 重要 Bug Fix | 数据损坏、内核崩溃、内存泄漏、死锁、生产可复现 |
| 普通 Bug Fix | Fixes: 标签、syzbot / Reported-by |
| 排除 | 纯清理/重构/NFC、纯文档/spelling、维护类（MAINTAINERS/mailmap）、"No functional change" |
...
- reviewer 返回 **PASS**：进入第五步
- reviewer 返回 **FAIL**：coordinator 针对不合格段落重跑对应 analyzer（只跑失败的那几条，不必全量重跑），拿到新文本后再次进入第五步，直到 PASS
